...
Для работы с проектом можно загрузить готовый многоархитектурный образ из GitHub Container Registry и тегировать его локально как \texttt{texlive}:

\begin{lstlisting}[language=bash]
# Загрузка и тегирование через Podman
...
# Загрузка и тегирование через Docker
...
В качестве альтернативы, образ можно собрать самостоятельно. В официальной документации приводится следующая команда для сборки напрямую из репозитория Git:

\begin{lstlisting}[language=bash]
podman build -t texlive https://github.com/denisstrizhkin/texlive-container.git
\end{lstlisting}

После успешного выполнения одной из этих команд в вашей системе появится готовый
